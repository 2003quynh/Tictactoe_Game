...
namespace DanielLochner.Assets.SimpleScrollSnap{
    ...
    public class SimpleScrollSnapEditor : SSSCopyrightEditor{
        ...
        private bool showMovementAndLayoutSettings = true,
            showNavigationSettings = true,
            showSnapSettings = true,
        ...
        private SerializedProperty movementType,
            ...
            sizeControl,
            size,
            automaticLayoutSpacing,
            automaticLayoutMargins,
            useInfiniteScrolling,
            infiniteScrollingSpacing,
            useOcclusionCulling,
            startingPanel,
            useSwipeGestures,
            minimumSwipeSpeed,
        ...
        private void ShowMovementAndLayoutSettings(){
            ...
            EditorLayoutUtility.Header(ref showMovementAndLayoutSettings,
                new GUIContent("Movement and Layout Settings"));
        ...
        private void ShowStartingPanel(){
            EditorGUILayout.IntSlider(startingPanel, 0, scrollSnap.NumberOfPanels - 1,
        ...
        private void ShowUseAutomaticLayout(){
            EditorGUILayout.PropertyField(useAutomaticLayout,
                new GUIContent("Use Automatic Layout",
            ...
            if (scrollSnap.UseAutomaticLayout) {
                ...
                EditorGUILayout.Slider(automaticLayoutSpacing, 0, 1,
        ...
        private void ShowUseInfiniteScrolling(){
            EditorGUILayout.PropertyField(useInfiniteScrolling,
                new GUIContent("Use Infinite Scrolling",
            ...
            if (scrollSnap.UseInfiniteScrolling) {
                EditorGUI.indentLevel++;
                EditorGUILayout.Slider(infiniteScrollingSpacing, 0, 1,
        ...
        private void ShowUseSwipeGestures(){
            EditorGUILayout.PropertyField(useSwipeGestures,
                new GUIContent("Use Swipe Gestures",
                    "Should users are able to use swipe gestures to navigate between panels?"));
            if (scrollSnap.UseSwipeGestures) {
                EditorGUI.indentLevel++;
                EditorGUILayout.PropertyField(minimumSwipeSpeed,
        ...
        private void ShowPreviousButton(){
            EditorGUILayout.ObjectField(previousButton, typeof(Button),
                new GUIContent("Previous Button", "(Optional) Button used to transition to the previous panel."));
        }

        private void ShowNextButton(){
            EditorGUILayout.ObjectField(nextButton, typeof(Button),
                new GUIContent("Next Button", "(Optional) Button used to transition to the next panel."));
        }

        private void ShowPagination(){
            EditorGUILayout.ObjectField(pagination, typeof(ToggleGroup),
                new GUIContent("Pagination",
                    "(Optional) ToggleGroup containing Toggles that shows the current position of the user and can be used to transition to a selected panel."));
            if (scrollSnap.Pagination != null) {
                EditorGUI.indentLevel++;
                EditorGUILayout.PropertyField(useToggleNavigation,
                    new GUIContent("Toggle Navigation",
                        "Should users be able to transition to panels by clicking on their respective toggle."));
                var numberOfToggles = scrollSnap.Pagination.transform.childCount;
                if (numberOfToggles != scrollSnap.NumberOfPanels)
                    EditorGUILayout.HelpBox(
                        "The number of toggles should be equivalent to the number of panels. There are currently " +
                        numberOfToggles + " toggles and " + scrollSnap.NumberOfPanels + " panels.",
                        MessageType.Warning);
                EditorGUI.indentLevel--;
            }
        }
        ...
        private void ShowSnapTarget(){
            ...
            if (scrollSnap.MovementType == MovementType.Free) scrollSnap.SnapTarget = SnapTarget.Nearest;
        }

        private void ShowSnapSpeed(){
            EditorGUILayout.PropertyField(snapSpeed,
                new GUIContent("Snap Speed", "The speed at which the targeted panel snaps into position."));
        }

        private void ShowThresholdSpeedToSnap(){
            EditorGUILayout.PropertyField(thresholdSpeedToSnap,
                new GUIContent("Threshold Speed To Snap",
        ...
        private void ShowUseHardSnapping(){
            EditorGUILayout.PropertyField(useHardSnapping,
                new GUIContent("Use Hard Snapping",
