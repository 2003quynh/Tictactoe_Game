...
namespace DanielLochner.Assets.SimpleScrollSnap{
    ...
    public class SimpleScrollSnap : MonoBehaviour, IDragHandler, IBeginDragHandler, IEndDragHandler,
        ...
        [SerializeField] private Vector2 size = new(400, 250);
        [SerializeField] private Margins automaticLayoutMargins = new(0);
        [SerializeField] private bool useInfiniteScrolling;
        [SerializeField] private float infiniteScrollingSpacing = 0.25f;
        [SerializeField] private bool useOcclusionCulling;
        [SerializeField] private int startingPanel;
        ...
        [SerializeField] private ToggleGroup pagination;
        ...
        // Events
        [SerializeField] private UnityEvent<GameObject, float> onTransitionEffects = new();
        [SerializeField] private UnityEvent<int> onPanelSelecting = new();
        [SerializeField] private UnityEvent<int> onPanelSelected = new();
        [SerializeField] private UnityEvent<int, int> onPanelCentering = new();
        [SerializeField] private UnityEvent<int, int> onPanelCentered = new();
        ...
        public UnityEvent<GameObject, float> OnTransitionEffects => onTransitionEffects;

        public UnityEvent<int> OnPanelSelecting => onPanelSelecting;

        public UnityEvent<int> OnPanelSelected => onPanelSelected;

        public UnityEvent<int, int> OnPanelCentering => onPanelCentering;

        public UnityEvent<int, int> OnPanelCentered => onPanelCentered;
        ...
        private bool ValidConfig {
            get {
                ...
                if (pagination != null) {
                    var numberOfToggles = pagination.transform.childCount;
        ...
        public void OnBeginDrag(PointerEventData eventData){
            if (useHardSnapping) ScrollRect.inertia = true;
        ...
        private void Setup(){
            ...
            // Panels
            if (sizeControl == SizeControl.Fit) size = Viewport.rect.size;
            ...
            for (var i = 0; i < NumberOfPanels; i++) {
                ...
                if (movementType == MovementType.Fixed && useAutomaticLayout) {
                    ...
                    var x = (automaticLayoutMargins.Right + automaticLayoutMargins.Left) / 2f -
                            automaticLayoutMargins.Left;
                    var y = (automaticLayoutMargins.Top + automaticLayoutMargins.Bottom) / 2f -
            ...
            if (movementType == MovementType.Fixed) {
                ...
                if (useAutomaticLayout) {
                    ...
                    var min = Panels[0].anchoredPosition;
                    var max = Panels[NumberOfPanels - 1].anchoredPosition;
                ...
                // Occlusion Culling
                if (useOcclusionCulling) HandleOcclusionCulling(true);
            }
            else {
            ...
            // Starting Panel
            var xOffset = movementType == MovementType.Free || movementAxis == MovementAxis.Horizontal
                ? Viewport.rect.width / 2f
                : 0f;
            var yOffset = movementType == MovementType.Free || movementAxis == MovementAxis.Vertical
            ...
            // Buttons
            if (previousButton != null) previousButton.onClick.AddListenerOnce(GoToPreviousPanel);
            if (nextButton != null) nextButton.onClick.AddListenerOnce(GoToNextPanel);
            ...
            if (pagination != null && NumberOfPanels != 0) {
                ...
                for (var i = 0; i < Toggles.Length; i++) {
                    ...
                    Toggles[i].onValueChanged.AddListenerOnce(delegate(bool isOn){
                        if (isOn && useToggleNavigation) GoToPanel(panelNumber);
                    });
                }
            }
        }
        ...
        private void HandleSelectingAndSnapping(){
            if (isSelected) {
                if (!((isDragging || isPressing) && useSwipeGestures)) SnapToPanel();
            }
            else if (!isDragging &&
        ...
        private void HandleInfiniteScrolling(bool forceUpdate = false){
            if (useInfiniteScrolling && (Velocity.magnitude > 0 || forceUpdate))
                switch (movementAxis) {
                    case MovementAxis.Horizontal:
                        for (var i = 0; i < NumberOfPanels; i++) {
                            var offset = new Vector2(contentSize.x, 0);
                    ...
                    case MovementAxis.Vertical:
                        for (var i = 0; i < NumberOfPanels; i++) {
                            var offset = new Vector2(0, contentSize.y);
        ...
        private void HandleTransitionEffects(){
            ...
            for (var i = 0; i < NumberOfPanels; i++) {
                var displacement = GetDisplacementFromCenter(i);
                var d = movementType == MovementType.Free ? displacement.magnitude :
        ...
        private void SelectPanel(){
            var nearestPanel = GetNearestPanel();
            var displacementFromCenter = GetDisplacementFromCenter(nearestPanel);
        ...
        private void SnapToPanel(){
            ...
            var targetPosition = -Panels[CenteredPanel].anchoredPosition + offset;
        ...
        public void GoToPanel(int panelNumber){
            ...
            if (pagination != null) Toggles[panelNumber].isOn = true;
            if (useHardSnapping) ScrollRect.inertia = false;
        }
        ...
        private int GetNearestPanel(){
            ...
            for (var i = 0; i < Panels.Length; i++) distances[i] = GetDisplacementFromCenter(i).magnitude;
        ...
        private void GetVelocity(){
            var displacement = Content.anchoredPosition - prevAnchoredPosition;
